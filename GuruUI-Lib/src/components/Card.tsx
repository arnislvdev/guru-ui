...
export const Card: React.FC<CardProps> = ({ title, description, children, footer, className }) => {
  return (
    <div className={`rounded-[var(--arnis-radius,0.5rem)] border border-white/10 bg-[#161616] p-6 shadow-lg ${className ?? ''}`.trim()}>
      {title ? (
        <h3 className="text-xl font-semibold text-[color:var(--arnis-heading,#ffffff)]">{title}</h3>
      ) : null}
      {description ? (
